...
# Study Sample Characteristics/ Demographics Table

```{r}
# Recode categorical variables for table presentation
dat = drug_data |>
  select(Age, Gender, Education, Country, Ethnicity) |>
  mutate_at(c("Age", "Gender", "Education", "Country", "Ethnicity"), as.factor) |>
  mutate(Gender = recode(Gender, "0" = "Male", 
         ...
         Education = recode(Education,
                            "Left school at 18 years" = "Left school at 18 years or less",
                            "Left school at 17 years" = "Left school at 18 years or less",
                            "Left school at 16 years" = "Left school at 18 years or less",
                            "Left school before 16 years" = "Left school at 18 years or less"),
         Ethnicity = recode(Ethnicity,
                            "Mixed-White/Black" = "Mixed",
                            "Mixed-White/Asian" = "Mixed",
                            "Mixed-Black/Asian" = "Mixed")
         )

# This is a fast way to compute the percentage of each level for a categorical variable
tab = dat |>
  count(Age) |>
  mutate(percent = scales::percent(n / sum(n)))

# Generating table using tbl_summary
tab = dat |>
  select(Age, Gender, Education, Country, Ethnicity) |>
  tbl_summary(label = list(Age ~ "Age Group",
                           Gender ~ "Gender",
              ...
              digits = ~ 2) |>
  modify_caption("**Table 1. Characteristics of study sample**") |>
  as_gt() |>
  gt::tab_source_note(gt::md("*Source: Fehrman et al, 2015*"))
tab
```
Interpretation: Most participants are under the age of 45. There are almost the same number of males and females. Most participants completed at least high school. Most participants are from the UK or USA. Most participants are white.
...
```
Interpretation: All OCEAN scores seem to show a normal distribution.
...
```
Interpretation: Impulsiveness scores seem to show a right skewed distribution.
...
```
Interpretation: Sensation seeking scores seem to show a left skewed distribution.
...
```
Interpretation: The distribution of Impulsiveness scores in drug users vs non-drug users are not significantly different. The median impulsivity score for non-users and users seem to be about the same. The impulsivity scores for drug users is has a larger third quartile than non-users. 
...
```
Interpretation: As impulsivity scores increase, the proportion of illegal drug users increase and the proportion of legal drug users decrease. As sensation seeking scores increase, the proportion of illegal drug users increase and the proportion of legal drug users decrease. This indicates that there seems to be a relationship between higher impulsivity and sensation seeking scores and being a illegal drug user.
